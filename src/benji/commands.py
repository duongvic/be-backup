...
class Commands:
    ...
    def db_seed(self) -> None:
        try:
            with Benji(self.config) as benji_obj:
                benji_obj.db_seed()
            logger.info('Seed data from miq successfully')
    ...
    def do_stress_test(self):
        from benji.database import Storage, VersionStatus, Version
        from benji.database import Database
        from benji.tasks import backup
        from benji import utils
        vm_id = 1
        volume_ids = ['khanhct', '500MB-data', '600MB-data', '3Lv7rPF1', '3Lv7rPFL', 'hours10',
                      'hours11', 'hours12', 'hours13', 'hours14', 'hours15', 'volume-khasn11hct']

        for volume_id in volume_ids:
            Database.configure(self.config)
            Database.open()
            storage = Storage.find_by_id(6)
            version_uid = '{}-{}'.format(utils.generate_uuid(), utils.random_string(6))
            source = 'rbd:volumes/volume-{}'.format(volume_id)
            version, err = Version(uid=version_uid, volume=volume_id, vm_id=vm_id,
                                   volume_name=utils.random_string(6), snapshot='', size=0, block_size=0,
                                   storage_id=storage.id, status=VersionStatus.incomplete, protected=False).create()
            node = dict(host="172.23.8.44", port=55051)
    ...
    def rest_api(self, bind_address: str, bind_port: int, threads: int) -> None:
        ...
        benji_app.config.from_object(self.config)
        logger.info(f'Starting REST API via gunicorn on {bind_address}:{bind_port}.')
