    ...
    def __init__(self, ad_hoc_config: str = None, sources: Sequence[str] = None) -> None:
        ...
        if self._CONFIGURATION_VERSION_KEY not in config:
            raise ConfigurationError(
                'Configuration is missing required key "{}".'.format(self._CONFIGURATION_VERSION_KEY))
...
class CeleryConfig(object):
    ...
    CELERYBEAT_SCHEDULE = {
        ...
        'delete-expired-versions-job': {
            'task': 'benji.tasks.schedule_job.delete_expired_versions',
            'schedule': timedelta(hours=1)
        },
        'monitor-nodes-job': {
            'task': 'benji.tasks.backup.monitor_nodes',
