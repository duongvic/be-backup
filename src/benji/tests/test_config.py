...
class ConfigTestCase(TestCaseBase, TestCase):
    ...
    def test_load_from_string(self):
        ...
        self.assertEqual('/var/log/benji.log', get('logFile', types=str))
        self.assertEqual(4194304, get('blockSize', types=int))

    def test_dict(self):
        config = Config(ad_hoc_config=self.CONFIG)
        nbd = get('nbd', types=dict)
        self.assertEqual('nbd', nbd.full_name)

    def test_lists(self):
        config = Config(ad_hoc_config=self.CONFIG)
        ios = get('ios', types=list)
    ...
    def test_defaults(self):
        config = Config(ad_hoc_config=self.CONFIG)
        self.assertEqual('benji', get('processName'))
        self.assertEqual('BLAKE2b,digest_bits=256', get('hashFunction'))

    def test_missing(self):
        config = Config(ad_hoc_config=self.CONFIG)
        self.assertRaises(KeyError, lambda: get('missing.option'))
    ...
    def test_load_from_file(self):
        ...
        self.assertEqual('/var/log/benji.log', get('logFile'))
    ...
    def test_validation_io_rbd(self):
        config = Config(ad_hoc_config=self.CONFIG)
        module_configuration = get('ios')[0]['configuration']
        self.assertEqual(
            {
                'cephConfigFile': '/etc/ceph/ceph.conf',
    ...
    def test_with_storage_id(self):
        ...
        storage_instance = get('storages')[0]
