...
from benji import sync_db
from benji.database import Database, VersionUid, Version, Block, User, \
    BlockUid, DereferencedBlock, VersionStatus, Storage, Locking, DeletedBlock, SparseBlockUid, Node, VM, User, TwoFactor
...
from benji.utils import notify, BlockHash, PrettyPrint, random_string
...
class Benji(ReprMixIn, AbstractContextManager):
    ...
    def __init__(self,
        ...
        if init_database:
            user = User(miq_id=0, fullname='Admin', email='admin', user_name='admin', status=True,
                        password='admin', enable_two_factors=False)
            user.create()
            ...
            storages = config.get('storages') or []
            for storage in storages:
                node = Node.raw_query().filter(Node.name == storage.pop('node')).one_or_none()
                if node:
                    storage['node_id'] = node.id
                    del storage['storageId']
                    new_storage = Storage(**storage)
                    new_storage.create()

        default_storage_name = self.config.get('defaultStorage', types=str)
        storage = Storage.raw_query().filter(Storage.name == default_storage_name).one_or_none()
        if not storage:
    ...
    def update_version(self, version: Version,  block_size: int = None, size: int = None,
                       base_version_uid: VersionUid = None, base_version_locking: bool = True):
        ...
        if base_version_uid:
            ...
            if old_version.storage != version.storage:
                raise UsageError(
                    f'Base version and new version have to be in the same storage'
                    f' ({old_version.storage.name} != {version.storage.name}).')
        ...
        version.block_size = new_version_block_size
    ...
    def create_version(self,
        ...
        if base_version_uid:
            ...
            if old_version.storage != new_storage:
                raise UsageError(
                    f'Base version and new version have to be in the same storage ({old_version.storage.name} != {new_storage.name}).')
    ...
    def _scrub_report_progress(self, *, version_uid: VersionUid, block: DereferencedBlock, read_jobs: int,
        ...
        logger.debug(
            '{} of block {} (UID {}) ok.'.format('Deep-scrub' if deep_scrub else 'Scrub', block.idx, block.uid))
        ...
        try:
            ...
            for entry in storage.read_get_completed():
                ...
                if source:
                    ...
                    if source_data != data:
                        logger.error(
                            'Source data has changed for block {} of version {} (UID {}) (is: {}... should-be: {}...). '
    ...
    def restore(self, version_uid: VersionUid, target: str, sparse: bool = False, force: bool = False) -> None:
        ...
        try:
            ...
            written = 0
            log_every_jobs = (
                                 sparse_blocks_count if not sparse else read_blocks_count) // 200 + 1  # about every half percent
    ...
    def rm(version_uid: VersionUid,
        ...
        with Locking.with_version_lock(version_uid, reason='Removing version', override_lock=override_lock):
            ...
            if not force:
                ...
                age_days = (datetime.datetime.now() - version.created_at).days
            ...
            if not keep_metadata_backup:
                try:
                    ...
                    logger.warning(
                        'Unable to remove version {} metadata backup from storage, the object was not found.'.format(
                            version_uid))
                    pass
    ...
    def backup(self,
        ...
        new_version_block_size = version.block_size if version.block_size != 0 else self._block_size

        io = IOFactory.get(source, block_size=new_version_block_size)
        io.open_r()

        source_size = io.size()
        self.update_version(version, new_version_block_size, source_size, base_version_uid)
        ...
        def do_create_backup():
            ...
            logger.info(
                f'Starting backup of {io.sanitized_url} to storage {version.storage.name}, '
                f'the newly created version is {version.uid}.')
            try:
                storage = StorageFactory.get_by_name(version.storage.name)
                read_jobs = 0
                for block in version.blocks:
    ...
    def clean(self, storage, dt: int = 3600, override_lock: bool = False):
        with Locking.with_lock(lock_name=f'Cleanup #{storage.name}',
                               reason=f'Cleanup #{storage.name}',
                               locked_msg='Another cleanup is already running.',
                               override_lock=override_lock):
            notify(self._process_name, f'Cleanup #{storage.name}')
            for hit_list in DeletedBlock.get_unused_block_uids(dt, storage.id):
    ...
    def export_data(*args, **kwargs):
        return Database.export_data(*args, **kwargs)
    ...
    @staticmethod
    def get_storage_id(storage_name: str):
        return Storage.get_storage_id(storage_name)
    ...
    def db_seed():
        #
        # TwoFactor.raw_query().delete()
        # VM.raw_query().delete()
        # User.raw_query().delete()
...
class BenjiStore(ReprMixIn):
    ...
    def create_cow_version(self, base_version: Version) -> Version:
        cow_version = self._benji_obj.create_version(
            ...
            snapshot=utils.get_local_time().isoformat(timespec='microseconds') + 'Z',
    ...
    def write(self, cow_version: Version, offset: int, data: bytes) -> None:
        ...
        for block, offset_in_block, length_in_block in write_list:
            if block is None:
                logger.warning(
                    'Tried to write data beyond device, it will be lost (version {}, size {}, offset {}).'.format(
