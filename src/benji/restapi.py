...
class RestAPI:
    ...
    def _api_v1_versions_list(
        self, volume: fields.Str(missing=None), status: fields.Str(missing=None),  
        from_date: fields.Str(missing=None), to_date: fields.Str(missing=None), 
        include_blocks: fields.Bool(missing=False),
    ) -> StringIO:
        ...
        if ('from_date' in request.query):
            from_date = request.query['from_date']
        if ('to_date' in request.query):
            to_date = request.query['to_date']    

        filter_str = ""
        if volume: 
            vol_str = "volume == '"+ volume +"' and "
            filter_str += vol_str
        if status: 
            stt_str = "status == '"+ status +"' and "
            filter_str += stt_str
        if from_date: 
            fdate_str = "date >= '"+ from_date +"' and "
            filter_str += fdate_str
        if to_date: 
            tdate_str = "date <= '"+ to_date +"' and "
            filter_str += tdate_str
        
        filter_str = filter_str[:-5]
        filter_expression = '('+ filter_str +')'

        # Check None all params 
